...
void disegnaTutto() {
  ...
  for (int i=0; i<lineaList.size(); i++) {
    ...
    PVector t1= lineaList.get(i).start;
    PVector t2=lineaList.get(i).end;
    
    pushMatrix();
    translate(-xOffset, -yOffset); //ritorna all'origine dello schermo
    scale(1/factor); //scala alla dimensione schermo
