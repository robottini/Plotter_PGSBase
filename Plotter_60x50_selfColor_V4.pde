...
float distHatch=1; //distanza tra inizio e fine del tratteggio e il bordo
...
void setup() {
  ...
  float distanza_hatching_carta = 2.0; // mm
  
  // Calcolo inverso: stepSVG (pixel schermo) = distanza_carta / factor
  // Assumendo che ridimPaper() applichi s.scale(factor) per andare da Schermo a Carta
  stepSVG = distanza_hatching_carta / factor;
  stepDisplay = stepSVG; // Per coerenza visiva
  
  // sovr = stepDisplay - 0.5;
  // Per migliorare la visualizzazione dell'hatching (evitando l'effetto "pieno"),
  // riduciamo lo spessore del tratto a video.
  sovr = 1.0; 

  println("-------------------------------------------------------------------");
  println("CALCOLO DISTANZA HATCHING (Target: " + distanza_hatching_carta + " mm)");
  println(" - Distanza su carta (Target): " + distanza_hatching_carta + " mm");
  println(" - Distanza algoritmo (stepSVG): " + stepSVG + " (unità interne/schermo)");
  println(" - Distanza output visivo: " + stepDisplay + " (unità interne/schermo)");
  println("-------------------------------------------------------------------");
